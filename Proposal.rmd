...
```{r}
#generate sample for us to work with
smple <- rjson::fromJSON(file = "0021500504.json")
full <- rjson::fromJSON(file = "0021500504.json")
smple$events <- smple$events[c(1:10)]
#smple with 10 events is created, much smaller so we can run more tests easily
...
#Playing with creating list of moments
#returns a list called 'moments' of all moments of all events
for (i in 1:length(smple$events)) {
  vector <- c(vector, smple$events[[i]][c('moments')])
}
vector <- vector[c(-1)]
moments <- vector

hello <- smple$events[[1]][[c('moments')]]





## WORKING ON CREATING DATA FRAME
df <- data.frame(matrix(ncol = 5, nrow = 0))
#this gets player data and ball data from one moment in the json file

#add in data from moments
# #TAKES TOO LONG
# for(x in 1:length(smple$events)) {
#   for(y in 1:length(smple$events[[x]][[c('moments')]])) {
#       for(lst in smple$events[[x]][[c('moments')]][[y]][[6]]) {
#         d <- data.frame(t(lst))
#         df <- rbind(d, df)
#       }
#     }
...
#works, but because data size is small
#upper one tries to do it with our sample data with 10 events,
#but it gets overloaded because it has to recreate every time
#kinda confused on how to make this work with apply functions
#or instead we can attempt to change the functions in his functions file to work

for(x in 1:length(smple$events)) {
  for(lst in smple$events[[x]][[c('moments')]][[1]][[6]]) {
        d <- data.frame(t(lst))
        df <- rbind(d, df)
    }
}
#name columns
colnames(df) <- c("teamId", "playerId", "xloc", "yloc", "zloc")
